...
def chat_with_retry(messages):
    for _ in range(MAX_RETRIES):
        try:
            ...
            print(e.status_code)
            print(e.response)
            if e.status_code == 429:    # 请求速率达到上限
                time.sleep(1)
                continue
            if e.status_code == 503:    # 服务器繁忙
                time.sleep(1)
                continue
            raise
...
def agent_loop(messages: list):
    ...
    # 失败场景：最大迭代次数
    print(f"\033[31mReached max steps\033[0m")
    ...
    for msg in reversed(messages):
        if msg["role"] == "assistant":
            ...
            if result:
                break
    if not result:
        result = f"Agent stopped after {MAX_STEPS} turns without final answer."
    messages.append({"role": "assistant", "content": [
    ...
    while True:
        ...
        for block in history[-1]["content"]:
            if getattr(block, "type", None) == "text":
                print(block.text)
            elif "type" in block and "text" in block:
                if block["type"] == "text":
                    print(block["text"])
        print()
